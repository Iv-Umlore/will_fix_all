...
- Тестирование необходимо проводить на сборке программного
...
__Ожидаемый результат__
    ...
    - Приложение не завершается в течение минуты (достаточно, чтобы удостовериться в том, что сервер останется включен и дальше).
___
...
__Необходимые требования__
    ...
    - Клиент сконфигурирован на правильный ip-адрес сервера.
...
#### 6.8.2 Список модульных тестов

- __SuccessRegistrationTest__ - Проверка успешной регистрации.
- __FailRegistrationTest__ - Проверка регистрации с ошибкой.
- __UserAutorizationTest__ - Проверка авторизации пользователем.
- __ManagerAutorizationTest__ - Проверка авторизации менеджером.
- __AdminAutorizationTest__ - Проверка авторизации администратором.
- __FailAutorizationTest__ - Проверка авторизации с ошибкой.
- __SendCalendarTest__ - Проверка передачи актуальной версии календаря.
- __SuccessSendCarInfoTest__ - Проверка успешной отправки информации о машине.
- __FailSendCarInfoTest__ - Проверка отправки информации о машине с  ошибкой.
- __SuccessSendRecordInfo__ - Проверка успешной отправки информации о заказе.
- __SuccessToBookATimeTest__ - Проверка успешной попытки бронирования времени.
- __FailToBookATimeTest__ - Проверка попытки бронирования занятого времени.
- __SuccessSendClientInfoToManagerTest__ - Проверка успешной отправки информации о клиентах менеджеру.
- __FailSendClientInfoToManagerTest__ - Проверка отправки информации о клиентах менеджеру с ошибкой.
- __SuccessChangeStatusTest__ - Проверка  успешного изменения статуса заказа.
- __FalseChangeStatusTest__ - Проверка изменения статуса заказа с ошибкой.
- __SuccessChangeTimeTest__ - Проверка успешного изменения времени заказа.
- __FalseChangeTimeTest__ - Проверка изменения времени заказа с ошибкой.
- __SuccessChangeManagerTest__ - Проверка успешного изменения менеджера, ответственного за заказ.
- __FalseChangeManagerTest__ - Проверка изменения менеджера, ответственного за заказ с ошибкой.
- __SuccessSetManagerTest__ - Проверка успешного назначения пользователя менеджером.
- __FalseSetManagerTest__ - Проверка назначение пользователя менеджером с ошибкой.
- __SuccessRemoveManagerTest__ - Проверка успешного понижения статуса менеджера до клиента.
- __FalseRemoveManagerTest__ - Проверка понижения статуса менеджера до клиента с ошибкой.
